...
# def test_search_for_product(driver_manager, login, product_data):
#     products_page = ProductsPage(driver_manager)
#     products_page.navigate_to_products_page()
#     product = product_data['products']['existing_products'][0]
#     products_page.search_for_product(product['name'], product['category'])
#     assert products_page.validate_search_result(product['name'], product['sku'], product['price'])

def test_add_new_product(driver_manager, login, product_data):
    products_page = ProductsPage(driver_manager)
    products_page.navigate_to_products_page()
    new_product = product_data['products']['new_products'][0]
    image_path = os.path.abspath(new_product['image_path'])
    products_page.add_new_product(new_product['name'], new_product['description'], new_product['category'],
                                   new_product['sku'], new_product['price'], image_path)
    assert products_page.validate_product_added(new_product['name'], new_product['category'], new_product['sku'], new_product['price'])
